...
local util = require("one-midnight.util")
...
function M.setup()
		...
		if color_name:sub(1, 1) == "$" then
			...
			if not color_name then
				vim.schedule(function()
					vim.notify('one-midnight.nvim: unknown color "' .. name .. '"', vim.log.levels.ERROR, { title = "one-midnight.nvim" })
				end)
				return ""
			end
		end
	...
	for group_name, group_settings in pairs(vim.g.one_midnight_config.highlights) do
		vim.api.nvim_command(
			string.format(
				"highlight %s %s %s %s %s",
				group_name,
